...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const { profile, heroImages, schedules, pastoralContacts, isLoading } = useChurch();
  ...
  // ===== TAMPILKAN LOADING (SETELAH SEMUA HOOKS!) =====
  if (isLoading) {
    return (
      <section id="beranda" className="relative min-h-[92vh] flex items-center justify-center bg-slate-900">
          ...
          <div className="lg:col-span-6 text-white space-y-6">
            ...
            <div className="space-y-3">
              <h1 className="text-3xl sm:text-5xl lg:text-5xl xl:text-6xl font-extrabold tracking-tight leading-tight text-white font-display">
                {profile?.name || 'GMAHK Salili Siau Tengah'}
              </h1>
              <p className="text-base sm:text-xl font-medium text-blue-200 max-w-xl leading-relaxed">
                {profile?.tagline || 'Mewartakan Kasih Karunia & Pengharapan Kedatangan Yesus'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs sm:text-sm text-blue-200 bg-white/10 backdrop-blur-md py-2.5 px-4 rounded-2xl border border-white/15 w-fit shadow-sm">
              ...
              <span className="font-medium">
                {profile?.village || 'Salili'}, Kec. {profile?.district || 'Siau Tengah'}, Kab. {profile?.regency || 'Sitaro'}, {profile?.province || 'Sulawesi Utara'}
              </span>
            </div>
              ...
              {primaryPastor && (
                <a
                  ...
                  href={`https://wa.me/${primaryPastor.whatsappNumber}?text=${encodeURIComponent(
                    primaryPastor.welcomeMessage || 'Halo, saya ingin berkonsultasi'
                  )}`}
                  target="_blank"
